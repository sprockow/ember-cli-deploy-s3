...
module.exports = CoreObject.extend({
  ...
  _putObjects: function(filePaths, options) {
    ...
    var cacheConfig      = options.cacheConfig;
    ...
    return pathsToUpload.map(function(filePath) {
      ...
      var fileCacheControl = cacheControl;
      var fileExpires = expires;

      console.log('Global Cache Control', fileCacheControl);
      console.log('Global Expires', fileExpires);

      if (typeof cacheConfig === "object") {
        var fileCacheConfig = _.find(cacheConfig, function(value, filePattern) {
           return minimatch(filePath, filePattern, { matchBase: true })
        });
        if (fileCacheConfig) {
          if (typeof fileCacheConfig.cacheControl !== 'undefined') {
            fileCacheControl = fileCacheConfig.cacheControl;
          }
          if (typeof fileCacheConfig.expires !== 'undefined') {
            fileExpires = fileCacheConfig.expires;
          }
        }
      }
      ...
      // if its explicitly false then
      if (fileCacheControl !== false) {
        params.cacheControl = fileCacheControl || defaultCacheControl;
      }

      // if its explicitly false then
      if (expires !== false) {
        params.expires = fileExpires || defaultExpires;
      }
